...
if (config.Version === "product") {
  ...
  dbl.webhook.on("vote", async (voter) => {
    ...
    const res = await fetch(`https://top.gg/api/users/${voter.user}`, {
      headers: {
    ...
    Rep.findOne(
      ...
      (err, rep) => {
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setAuthor(
            tag,
            `https://cdn.discordapp.com/avatars/${voter.user}/${body.avatar}.png`
          )
          .setThumbnail(guild.iconURL({ dynamic: true }))
          .setDescription(
            `**${tag}** Has upvoted KFC Bucket Boy over at <https://top.gg/bot/614110037291565056> \n\n And they have received ${addedrep} Rep! <:chickennuggie:706268265424355399>`
          )
...
console.log("express is launched!");
...
bot.on("message", (message) => {
  ...
  if (usedCommandRecentllytext.has(message.author.id)) {
    message.reply(
      "Woah there you can only use me so fast <a:kaineflushedeyes:708477282079211570> 3 seconds per command"
    );
  } else {
      ...
      if ((isuser = true)) {
        ...
        if (user) {
          ...
          if (UserArgs[1] === undefined) {
            ...
            var theUser = message.guild.members.cache.find(
              (member) => member.id === UserArgs[1]
            );
          }
        }
      }

      var realuser = message.guild.members.cache.filter(
