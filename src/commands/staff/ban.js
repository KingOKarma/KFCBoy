...
module.exports = {
    ...
    run: (_, message, args, prefix, MongoToggle, theUser) => {
        ...
        if (!message.member.permissionsIn(message.channel).has("BAN_MEMBERS")) return message.channel.send("You need the permission __**\"Ban Members\"**__ to use this command")
        if (!message.guild.me.permissionsIn(message.channel).has("BAN_MEMBERS")) return message.channel.send("I need the permission __**\"Ban Members\"**__ to use this command")
        ...
        Toggle.findOne({
            ...
            (err, toggle) => {
                ...
                if (!toggle) {
                    ...
                    if (!theUser.bannable) return message.channel.send("I cannot Ban members who have ban permissions!")
                    theUser.ban(`${theUser} was banned! for ${reason}`)
                        .then(() => {
                            Log.findOne({
                                ServerID: message.guild.id,
                                ...
                                (err, log) => {
                                    ...
                                    if (log) {
                                        ...
                                        message.channel.send("First time this user has been banned from this server! <:Kainepog:709455703567499326>")
                        ...
                        .then(() => {
                            message.reply(`**${theUser.user.tag}** Has been banned! for ${reason}`)
                        })
